 ...
 * 返回参数：{ userInfo: object, token: string, roleType: string }
 * url地址：/user/login
 * 请求方式：POST
 */
 ...
 * 用户注册
 * 功能描述：新用户注册账号
 * 入参：{ username: string, password: string, confirmPassword: string, email: string, phone?: string, name?: string, userType?: string }
 * 返回参数：注册成功信息
 * url地址：/user/add
 * 请求方式：POST
 */
 ...
 * 功能描述：获取当前登录用户的详细信息
 * 入参：无
 * 返回参数：{ id: number, username: string, nickname: string, email: string, phone: string, sex: string, avatar: string, userType: string }
 * url地址：/user/current
 * 请求方式：GET
 */
