...
public class FileService {
     ...
     * 上传文件并绑定业务对象（支持替换）
     */
    public FileInfoDTO uploadFile(MultipartFile file, FileUploadDTO uploadDTO, boolean replaceOld) {
        ...
        try {
            ...
            // 3. 先保存文件到磁盘（在事务外，避免事务影响文件IO）
            filePath = FileUtil.saveFile(file, BUSSINESS_FILE_RELATIVE_PATH, FileUtil.parseBussinessFileTypeToFolerName(uploadDTO.getBusinessType()));
            
            // 检查文件是否保存成功
            if (filePath == null || filePath.isEmpty()) {
                throw new BusinessException("文件保存失败，请检查文件格式和服务器配置");
            }
            
            log.info("文件保存到磁盘成功: {}", filePath);

            // 4. 在事务中处理数据库操作
            return saveFileInfoInTransaction(file, uploadDTO, filePath, replaceOld);
    ...
    private FileInfoDTO saveFileInfoInTransaction(MultipartFile file, FileUploadDTO uploadDTO, String filePath, boolean replaceOld) {
        try {
            // 1. 如果需要替换，先处理旧文件
            if (replaceOld) {
            ...
            // 2. 保存文件信息到数据库
            SysFileInfo fileInfo = createFileInfo(file, uploadDTO, filePath);
            fileInfoMapper.insert(fileInfo);

            log.info("文件信息保存到数据库成功: ID={}, 路径={}", fileInfo.getId(), filePath);
            return convertToDTO(fileInfo);
            
        } catch (Exception e) {
            log.error("保存文件信息失败，将删除已上传的文件: {}", filePath);
            // 如果数据库操作失败，删除已保存的文件
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public FileInfoDTO uploadTempFile(MultipartFile file) {
        try {
            ...
            // 检查文件是否保存成功
            if (filePath == null || filePath.isEmpty()) {
                throw new BusinessException("临时文件保存失败，请检查文件格式和服务器配置");
            }

            // 创建临时文件记录
            FileUploadDTO tempDTO = buildTempUploadDTO();
            SysFileInfo fileInfo = createFileInfo(file, tempDTO, filePath);
    ...
    public boolean deleteFilesByBusiness(String businessType, String businessId, String businessField) {
        try {
            ...
            // 直接删除数据库记录
            int result = fileInfoMapper.delete(queryWrapper);
     ...
     * 创建文件信息对象
     */
    private SysFileInfo createFileInfo(MultipartFile file, FileUploadDTO uploadDTO, String filePath) {
